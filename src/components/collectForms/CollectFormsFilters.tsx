...
export type CollectFilter = "ALL" | "TEMPLATE" | "CUSTOM";
...
interface Props {
  activeFilter: CollectFilter;
  onChange: (filter: CollectFilter) => void;
  resultCount: number;
  totalCount: number;
  loading?: boolean;
}

export default function CollectFormsFilters({
  activeFilter,
  onChange,
  resultCount,
  totalCount,
            ...
            <p className="text-[12px] text-[#64748B]">
              {loading ? (
                ...
                <>
                  ...
                  de {totalCount}{" "}
                  {totalCount === 1 ? "formulario" : "formularios"}
                </>
              )}
            </p>
          </div>
